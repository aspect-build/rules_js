...
#
# It goes at the very top of the launcher, ahead of the environment block, because
# a js_binary `env` value may reference $BAZEL_BINDIR and setEnv expands those
# references as it goes: a value set further down would come too late. This is
# where the bash launcher set it too, from a preamble that replaced the shebang.
_BAZEL_BINDIR_PATCH = """\
'use strict'
...
# The build-time bindir, on the other hand, is patched in place: it is only ever
# read after the environment block has run.
_BINDIR_PATCH = 'setEnv("JS_BINARY__BINDIR", process.cwd())'

def _launcher_js(binary):
    """The JavaScript launcher generated by js_binary.

    Fails rather than returning None: without it the launcher keeps the build-time
    bindir and target cpu baked in, which does not fail the build and does not fail
    the layer -- it fails at container run time, with a message that says nothing
    about the layer that produced it.
    """
    launchers = []
    if OutputGroupInfo in binary and hasattr(binary[OutputGroupInfo], "launcher_js"):
        launchers = binary[OutputGroupInfo].launcher_js.to_list()
    if len(launchers) != 1:
        fail("js_image_layer binary %s does not provide exactly one `launcher_js` output group file; `binary` must be a js_binary or a rule built on one, such as js_run_devserver." % binary.label)
    return launchers[0]

def _write_launcher_js(ctx, launcher_js):
    """Sanitizes the js_binary JavaScript launcher for use inside a container.

    The launcher bakes in values that are correct in the Bazel output tree but not in the
    image: the bindir path, and the target cpu of the build rather than of the machine
    running the container. Rewrite those to be computed at runtime, and set BAZEL_BINDIR
    so the launcher stays in the runfiles tree rather than trying to cd to a bindir.
    """
    launcher = ctx.actions.declare_file("%s_launcher.cjs" % ctx.label.name)

    substitutions = {
        "'use strict'": _BAZEL_BINDIR_PATCH,
        'setEnv("JS_BINARY__BINDIR", "%s")' % launcher_js.root.path: _BINDIR_PATCH,
        'setEnv("JS_BINARY__TARGET_CPU", "%s")' % ctx.expand_make_variables("", "$(TARGET_CPU)", {}): 'setEnv("JS_BINARY__TARGET_CPU", os.machine())',
    }
    substitutions['setEnv("JS_BINARY__BINDIR", "%s")' % ctx.bin_dir.path] = _BINDIR_PATCH
...
def _js_image_layer_impl(ctx):
    ...
    executable = binary_default_info.files_to_run.executable

    # The js_binary executable is a hermetic native launcher that only embeds runfiles-relative
    # paths, so it needs no sanitizing; the JavaScript launcher it execs does.
    launcher_js = _launcher_js(ctx.attr.binary[0])
    launcher = _write_launcher_js(ctx, launcher_js)
    ...
    entries.add("{")
    entries.add_joined(
        [binary_path, {"dest": executable.path, "root": executable.root.path}],
        join_with = ":",
    ...
    # The JavaScript launcher generated by js_binary contains non-reproducible information;
    # swap the runfiles copy out with the sanitized one.
    entries.add(",")
    entries.add_joined(
        [runfiles_dir + "/" + _to_rlocation_path(launcher_js, workspace_name), {"dest": launcher.path, "root": launcher.root.path}],
        join_with = ":",
        map_each = json.encode,
    )
    ...
    for typ, mtree, unused_inputs in layer_groups_gen:
        ...
        ctx.actions.run(
            inputs = depset(
                ([repo_mapping] if repo_mapping else []) + [launcher, entries_json, mtree, unused_inputs],
