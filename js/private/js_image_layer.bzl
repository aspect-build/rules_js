...
setEnv("BAZEL_BINDIR", ".")
setEnv("JS_BINARY__BINDIR", process.cwd())"""

def _launcher_js(binary):
    """The JavaScript launcher generated by js_binary, or None for a non-js_binary target."""
...
def _js_image_layer_impl(ctx):
    ...
    launcher = _write_launcher_js(ctx, launcher_js) if launcher_js else None
    ...
    for typ, mtree, unused_inputs in layer_groups_gen:
        ...
        ctx.actions.run(
            inputs = depset(
                ([repo_mapping] if repo_mapping else []) + ([launcher] if launcher else []) + [entries_json, mtree, unused_inputs],
