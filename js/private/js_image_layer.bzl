...
js_binary_lib.create_launcher that republishes its launcher_js in an output group:
    ...
    if not launchers:
        ...
        return None
    if len(launchers) != 1:
        fail("expected {} to have exactly one launcher_js file, got {}".format(binary.label, launchers))
    return launchers[0]
...
def _js_image_layer_impl(ctx):
    ...
    entries.add("{")
    entries.add_joined(
        [binary_path, {"dest": entry_point_file.path, "root": entry_point_file.root.path}],
        join_with = ":",
    ...
    # The launcher generated by js_binary contains non-reproducible information; swap it out
    # with the sanitized one.
    binary_path_under_runfiles = runfiles_dir + "/" + _to_rlocation_path(sanitized_original, workspace_name)
    entries.add_joined(
