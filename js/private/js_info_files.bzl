...
def _js_info_files_impl(ctx):
    return DefaultInfo(files = _gather_files_from_js_info(
        ...
        include_npm_deps = ctx.attr.include_npm_deps,
    ))
...
js_info_files = rule(
    ...
    attrs = {
        ...
        "include_npm_deps": attr.bool(
            doc = """When True, files in `npm_deps` from `JsInfo` providers in srcs targets are included in the default outputs of the target.
