...
def _js_run_binary_extra_tool_data_impl(ctx):
    tool_info = ctx.attr.tool[DefaultInfo]

    # Include both default outputs (DefaultInfo.files) and default_runfiles of each
    # extra_data target. Library targets (e.g. js_library) put source files in
    # DefaultInfo.files rather than in default_runfiles, so both must be collected.
    extra_runfiles = ctx.runfiles(
        transitive_files = depset(transitive = [
            target[DefaultInfo].files
            for target in ctx.attr.extra_data
        ]),
    ).merge_all([
        target[DefaultInfo].default_runfiles
        for target in ctx.attr.extra_data
    ])
    combined_runfiles = tool_info.default_runfiles.merge(extra_runfiles)

    # Bazel requires that an executable rule creates its own executable. We create a
    # symlink to the original tool to satisfy this constraint while preserving behavior.
...
_js_run_binary_extra_tool_data = rule(
    implementation = _js_run_binary_extra_tool_data_impl,
    attrs = {
        "tool": attr.label(executable = True, cfg = "exec"),
        "extra_data": attr.label_list(allow_files = True),
    },
    executable = True,
)
...
def js_run_binary(
    ...
    * BAZEL_WORKSPACE: the bazel repository name; equivalent to `ctx.workspace_name` of the `js_run_binary` target's rule context
    ...
    Args:
        ...
        extra_tool_data: Additional data to include in the runfiles of `tool` when running this action.

            Use this to pass files (such as a JavaScript config file) that must be resolved as
            imports from within the `tool`'s runfiles tree. When `use_execroot_entry_point` is
            False, all of the tool's sources live in the runfiles directory; a config file passed
            here will be placed alongside them so that its `require`/`import` calls resolve against
            the same `node_modules` tree. Prefer this over adding the file to the `data` attribute
            of the `js_binary` tool target, which would make the config a permanent dependency of
            the binary rather than an input to this specific build action.
    ...
    # If extra_tool_data is provided, wrap the tool with a target that merges the extra data
    # into its runfiles. The resulting target keeps the same executable so the runfiles directory
    # path is unchanged; only its contents are augmented.
    effective_tool = tool
    augmented_tool_name = ""
    if extra_tool_data:
        augmented_tool_name = "_{}_extra_tool_data".format(name)
        _js_run_binary_extra_tool_data(
            name = augmented_tool_name,
            tool = tool,
            extra_data = extra_tool_data,
            tags = kwargs.get("tags", []) + ["manual"],
            testonly = kwargs.get("testonly", False),
        )
        effective_tool = ":{}".format(augmented_tool_name)
...
""".format(
    ...
    # When extra_tool_data is used, the augmented wrapper must also appear in 'tools' so that
    # $(execpath :_{name}_extra_tool_data) is available for path construction in args/env.
    if extra_tool_data:
        extra_kwargs = dict(kwargs)
        extra_kwargs["tools"] = list(kwargs.get("tools", [])) + [":{}".format(augmented_tool_name)]
    else:
        extra_kwargs = kwargs
