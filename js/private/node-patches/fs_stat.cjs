...
class FsInternalStatPatcher {
    ...
    patch() {
        ...
        internalFs.lstat = function (path, bigint, reqCallback, throwIfNoEntry) {
            ...
            if (!needsGuarding) {
                return internalFsLStat.apply(internalFs, arguments);
            }
            if (reqCallback === internalFs.kUsePromises) {
                return internalFsLStat
                    ...
                    return new Promise((resolve, reject) => {
                        statPatcher.eeguardStats((0, fs_cjs_1.resolvePathLike)(path), bigint, stats, !!throwIfNoEntry, (err, guardedStats) => {
                            err
                                ? reject(err)
                ...
                const stats = internalFsLStat.apply(internalFs, arguments);
                if (!stats) {
                    return stats;
                }
                return statPatcher.eeguardStatsSync((0, fs_cjs_1.resolvePathLike)(path), bigint, !!throwIfNoEntry, stats);
            }
            else {
    ...
    eeguardStats(path, bigint, stats, throwIfNotFound, cb) {
        ...
        return this.guardedReadLink(path, (str) => {
            ...
            return this.unguardedRealPath(path, (err, str) => {
                ...
                return internalFsLStat.call(internalFs, str, bigint, req2 // TODO: why type mismatch here?
                );
            });
        });
    }
