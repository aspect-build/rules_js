...
# {{coverage_entry_point}} is empty for js_binary (non-test) targets.
if [ "$TEST_PASSED" = true ] && [ -n "{{coverage_entry_point}}" ] && [ "${COVERAGE_DIR:-}" ]; then
    JS_COVERAGE__RUNFILES="$JS_BINARY__RUNFILES" \
        "$JS_BINARY__NODE_BINARY" "$JS_BINARY__RUNFILES/{{workspace_name}}/{{coverage_entry_point}}" || {
