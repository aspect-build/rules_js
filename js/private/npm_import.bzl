...
```
load("@npm__types_node-15.2.2//:js_package.bzl", js_package_types_node = "js_package")

js_package_types_node()
```
...
```
load("@npm//:js_packages.bzl", "js_packages")

js_packages()
```

`translate_pnpm_lock` also creates convienence aliases in the external repository that reference
the linked `js_package` targets. For example, `@npm//name` and `@npm//@scope/name`.
...
"""def js_package():
    if "{link_package_guard}" != "." and native.package_name() != "{link_package_guard}":
        fail("The js_package() macro loaded from {js_package_bzl} may only be called in the '{link_package_guard}' package. Move the call to the '{link_package_guard}' package BUILD file.")
...
def _impl(rctx):
    ...
    js_package_bzl_file = "js_package.bzl"

    bazel_name = npm_utils.bazel_name(rctx.attr.package_name, rctx.attr.package_version)
    js_package_tmpl = _NODEJS_PACKAGE_EXPERIMENTAL_REF_DEPS_TMPL if rctx.attr.experimental_reference_deps else _NODEJS_PACKAGE_TMPL
    js_package_bzl = [js_package_tmpl.format(
        namespace = npm_utils.js_package_target_namespace,
        dir = dirname,
        ...
        rctx_name = rctx.name,
        js_package_bzl = "@%s//:%s" % (rctx.name, js_package_bzl_file),
    ...
    if not rctx.attr.indirect:
        js_package_bzl.append(_ALIAS_TMPL.format(
    ...
    bzl_header = [
        ...
        """load("@aspect_rules_js//js:js_package.bzl", _js_package = "js_package")""",
        "",
    ]
    rctx.file(js_package_bzl_file, "\n".join(bzl_header + js_package_bzl))
...
_ATTRS = {
    ...
    "link_package_guard": attr.string(
        doc = """When explictly set, check that the generated js_package() marcro
        ...
        to guard against linking the generated js_packages into the wrong
        location.""",
        default = ".",
    ),
