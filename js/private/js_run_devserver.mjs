...
async function syncFiles(files, sandbox, writePerm, recurse = syncRecursive) {
    ...
    let totalSynced = (
        await Promise.all(
            otherFiles.map(async (file) => {
                ...
                return await recurse(src, dst, sandbox, writePerm)
            })
        )
    ).reduce((s, t) => s + t, 0)
...
async function watchProtocolCycle(config, configPath, sandbox, cycle) {
    ...
    // Also index the cycled sources with the BINDIR removed.
    // TOOD: are there any paths without the BINDIR?
    Object.keys(cycle.sources).forEach((f) => {
        if (f.startsWith(process.env.JS_BINARY__BINDIR + '/')) {
            cycle.sources[f.slice(process.env.JS_BINARY__BINDIR.length + 1)] =
                cycle.sources[f]
        }
    })

    const filesToSync = newFiles.filter((f) => cycle.sources.hasOwnProperty(f))

    console.error(`js_run_devserver[WATCH]: Syncing `, filesToSync)
...
async function cycleSyncRecurse(src, dst, sandbox, writePerm) {
    // TODO: this was originally a copy of syncRecursive, but when 'src' is always new/needs-update a
    // lot of the "if changed" checks are not needed, and more can still be removed.
    try {
        const lstat = await withRetry(
            () => fs.promises.lstat(src),
            `lstat for ${src}`
...
class AspectWatchProtocol {
    ...
    async connect() {
        ...
        await this._send('NEGOTIATE_RESPONSE')
    ...
    async cycle() {
        ...
        } while (this.connection.readable && this.connection.writable)
    }
