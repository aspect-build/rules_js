...
async function watchProtocolCycle(config, configPath, sandbox, cycle) {
    ...
    Object.keys(cycle.sources).forEach((f) => {
        if (f.startsWith('_main/')) {
            cycle.sources[f.slice('_main/'.length)] = cycle.sources[f]
        }
    })
...
async function cycleSyncRecurse(cycle, src, dst, sandbox, writePerm) {
    ...
    const srcWorkspacePath = src.startsWith(RUNFILES_ROOT)
        ? src.slice(RUNFILES_ROOT.length + 1)
        : src

    if (!(srcWorkspacePath in cycle.sources)) {
        throw new Error(
            `File ${srcWorkspacePath} is not in the cycle sources, cannot sync it.`
        )
    }

    if (cycle.sources[srcWorkspacePath].is_symlink) {
        let symlinkMeta = ''
        if (isNodeModulePath(src)) {
            let linkPath = await fs.promises.readlink(src)
            if (path.isAbsolute(linkPath)) {
    ...
    // TODO: watch protocol needs an is_directory
    const lstat = await withRetry(
        () => fs.promises.lstat(src),
        `abazel lstat for ${src}`
    )
    ...
    if (lstat.isDirectory()) {
        ...
        if (process.env.JS_BINARY__LOG_DEBUG) {
            console.error(`Syncing file ${src.slice(RUNFILES_ROOT.length + 1)}`)
        }
        if (exists) {
            await fs.promises.unlink(dst)
        } else {
        ...
        await withRetry(
            () => fs.promises.copyFile(src, dst),
            `abazel copyFile from ${src} to ${dst}`
        )
