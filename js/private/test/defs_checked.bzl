...
def link_js_packages():
    ...
    root_path = ""
    importer_paths = [".", "examples/lib", "examples/npm_deps", "js/private/test"]
    ...
    if not is_root and not is_direct:
        msg = "The link_js_packages() macro loaded from @npm//:defs.bzl and called in bazel package '%s' may only be called in the bazel package(s) corresponding to the root package '' and packages corresponding to importer paths ['.', 'examples/lib', 'examples/npm_deps', 'js/private/test']" % native.package_name()
        fail(msg)
