...
def rollup(name, **kwargs):
    _directory_path(
        ...
        directory = "@//examples/npm_deps:direct__rollup__2.70.2__dir",
        path = "dist/bin/rollup",
    )
    _js_binary(
        name = "%s__js_binary" % name,
        entry_point = ":%s__entry_point" % name,
        data = ["@//examples/npm_deps:direct__rollup__2.70.2"],
    )
    _run_js_binary(
        name = name,
...
def rollup_test(name, **kwargs):
    ...
    _js_test(
        name = name,
        entry_point = ":%s__entry_point" % name,
        data = kwargs.pop("data", []) + ["@//examples/npm_deps:direct__rollup__2.70.2"],
        **kwargs
    )
