...
def _impl(rctx):
    ...
    timeresult = rctx.execute(["date"])
    time = timeresult.stdout
    generated_by_lines = [
        "\"@generated by @aspect_rules_js//js/private:translate_pnpm_lock.bzl from pnpm lock file @{pnpm_lock_wksp}{pnpm_lock} at {time}\"".format(
