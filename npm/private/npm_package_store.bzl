...
def _npm_package_store_impl(ctx):
    ...
    if not ctx.attr.key:
        fail("No package key specified to link to. Package key must be specified explicitly via 'key' attribute")
...
def npm_local_package_store_internal(package_store_name, key, package, version, src, deps, visibility, tags):
