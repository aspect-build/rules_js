...
deps of npm_package_store must be in the same package.""" % (ctx.label.package, dep[NpmPackageStoreInfo].root_package)
    ...
    if ctx.attr.src:
        ...
        # npm_package_store_infos is the transitive closure of the all NpmPackageStoreInfos in deps,
        # we can safely add just `files` from each of these to `transitive_files_depset`. Doing so
