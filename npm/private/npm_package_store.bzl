...
""",
    ...
    # Known generation path: "imported" (npm_import), "local" (workspace store), or "" (unspecified).
...
deps of npm_package_store must be in the same package.""" % (ctx.label.package, dep_info.root_package)
    ...
    if js_runfiles_groups.is_enabled(ctx):
        providers.extend(_npm_package_store_groups(
            ctx,
            files = files,
            package_store_directory = package_store_directory,
            src = ctx.attr.src,
            deps = ctx.attr.deps,
            default_runfiles = ctx.attr.src[DefaultInfo].default_runfiles if ctx.attr.src else None,
        ))

    return providers

def _npm_package_store_groups(ctx, files, package_store_directory, src, deps, default_runfiles):
    kind, rank = js_runfiles_groups.store_kind_and_rank(ctx.attr._store_provenance)

    # Terminal (no src) stores re-list the real store's package directory to
    # publish it in DefaultInfo. That directory is already owned by the store
    # that created it; grouping it again under the /pkg Label overlaps.
    own_files = files
    if not src and package_store_directory:
        own_files = [f for f in files if f != package_store_directory]
    own = js_runfiles_groups.maybe_files_entry(ctx.label, own_files, kind, rank)
    own_list = [own] if own else []
    dep_targets = deps.keys() if type(deps) == "dict" else deps
    trans_field = "transitive_store_files" if src else "store_files"
    fallback_field = "transitive_files" if src else "files"

    trans_direct = list(own_list)
    trans_trans = []
    for dep in dep_targets:
        if js_runfiles_groups.JsRunfilesGroupsInfo in dep:
            trans_trans.append(getattr(dep[js_runfiles_groups.JsRunfilesGroupsInfo], trans_field))
        else:
            files = getattr(dep[NpmPackageStoreInfo], fallback_field)
            if files:
                trans_direct.append(js_runfiles_groups.fallback_entry(dep.label, files))

    src_list = [src] if src else []
    js_src_field = "transitive_sources" if src else "sources"
    js_typ_field = "transitive_types" if src else "types"
    companion = js_runfiles_groups.JsRunfilesGroupsInfo(
        sources = js_runfiles_groups.channel_entries_from_targets(src_list + dep_targets, js_src_field, js_src_field),
        types = js_runfiles_groups.channel_entries_from_targets(src_list + dep_targets, js_typ_field, js_typ_field),
        transitive_sources = js_runfiles_groups.channel_entries_from_targets(src_list + dep_targets, js_src_field, js_src_field),
        transitive_types = js_runfiles_groups.channel_entries_from_targets(src_list + dep_targets, js_typ_field, js_typ_field),
        npm_sources = js_runfiles_groups.channel_entries_from_targets(
            src_list + dep_targets,
            "npm_sources",
            "npm_sources",
            own = own_list,
        ),
        store_files = js_runfiles_groups.entries(direct = own_list),
        transitive_store_files = js_runfiles_groups.entries(direct = trans_direct, transitive = trans_trans),
    )
    ...
    if src:
        if js_runfiles_groups.RunfilesGroupInfo in src:
            ...
            rf = default_runfiles if default_runfiles != None else ctx.runfiles()
            runtime_own.append(js_runfiles_groups.fallback_entry(src.label, rf))
...
# Workspace-local stores generated by npm_link_all_packages. Same impl as
# npm_package_store; the private provenance default is first_party.
npm_local_package_store = rule(
    implementation = npm_package_store_lib.implementation,
    attrs = npm_package_store_lib.attrs | {
        "_store_provenance": attr.string(
            values = ["", "imported", "local"],
