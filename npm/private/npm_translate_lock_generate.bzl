...
# buildifier: disable=function-docstring
def generate_repository_files(rctx, importers, packages, snapshots, patched_dependencies, only_built_dependencies, root_package, default_registry, npm_registries, npm_auth):
    ...
    npm_imports = helpers.get_npm_imports(importers, packages, snapshots, rctx.attr.replace_packages, patched_dependencies, only_built_dependencies, root_package, rctx.name, rctx.attr, rctx.attr.lifecycle_hooks, rctx.attr.lifecycle_hooks_execution_requirements, rctx.attr.lifecycle_hooks_use_default_shell_env, npm_registries, default_registry, npm_auth)
...
""",
    ...
    for link_package, import_path in package_to_importer.items():
        ...
        for dep_name, dep_key in importer.get("all_deps", {}).items():
            ...
            if dep_key.find("@file:") != -1:
                ...
                if not fp_links.get(dep_key, False):
                    ...
                    if importers.get(dep_link, False):
                        raw_deps = importers[dep_link]["deps"]
...
""".format(
    ...
    for i, fp_key in enumerate(fp_links.keys()):
        fp_link = fp_links[fp_key]
