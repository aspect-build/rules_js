...
def generate_repository_files(rctx_name, attr, importers, packages, patched_dependencies, only_built_dependencies, root_package, default_registry, npm_registries, npm_auth):
    ...
    npm_fp_links, npm_tp_links = helpers.get_npm_links(importers, packages, root_package)
...
""",
    ...
    npm_package_locations = _generate_npm_package_locations(npm_fp_links, npm_imports, npm_tp_links) if has_package_visibility else None
...
""".format(
    ...
    for (i, _import) in enumerate(npm_imports):
        link_packages = npm_tp_links.get(_import.package_key, {})
        if link_packages:
        ...
        for import_path, link_aliases in link_packages.items():
            link_package = helpers.link_package(root_package, import_path)
            link_importer = importers.get(import_path)
            ...
            for link_alias in link_aliases.keys():
    ...
    for i, fp_path in enumerate(npm_fp_links.keys()):
        fp_link = npm_fp_links[fp_path]
        fp_package = fp_link["name"]
        ...
        # Convert the deps to package store targets
        fp_deps = {}
        for raw_package, raw_version in fp_link["deps"].items():
            ...
            if dep_store_target not in fp_deps:
                fp_deps[dep_store_target] = [raw_package]
            else:
                fp_deps[dep_store_target].append(raw_package)

        # collapse link aliases lists into to acomma separated strings
        for dep_store_target in fp_deps.keys():
            fp_deps[dep_store_target] = ",".join(fp_deps[dep_store_target])
        ...
        if fp_link["fp_type"] == "importer" and fp_link["link_packages"]:
...
def _generate_npm_package_locations(fp_links, npm_imports, npm_tp_links):
    ...
    # Add npm imports
    # for pk, _import in npm_imports.items():
    #     for link_package, link_aliases in npm_tp_links.get(pk, {}).items():
