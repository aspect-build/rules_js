...
def _rctx_write_file(rctx, rctx_out, filename, contents):
    if filename in rctx_out:
        fail("ERROR: duplicate file write: %s" % filename)

    rctx_out[filename] = True
...
""".format(
    ...
    for (i, _import) in enumerate(npm_imports):
        ...
        for link_package in _import.link_packages.keys():
            ...
            _rctx_write_file(rctx, rctx_out, resolved_json_file_path, json.encode({
    ...
    for filename, contents in rctx_files.items():
        _rctx_write_file(rctx, rctx_out, filename, generated_by_prefix + "\n" + "\n".join(contents))
