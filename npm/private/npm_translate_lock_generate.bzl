...
_FP_DIRECT_TMPL = \
    ...
    if bazel_package in {link_packages}:
        ...
        _npm_link_package_store(
            name = "{{}}/{pkg}".format(name),{maybe_package_alias}
...
def generate_repository_files(rctx, pnpm_lock_label, importers, packages, snapshots, patched_dependencies, only_built_dependencies, root_package, default_registry, npm_registries, npm_auth, link_workspace):
    ...
    npm_imports = helpers.get_npm_imports(importers, packages, snapshots, patched_dependencies, only_built_dependencies, root_package, rctx.name, rctx.attr, rctx.attr.lifecycle_hooks, rctx.attr.lifecycle_hooks_execution_requirements, rctx.attr.lifecycle_hooks_use_default_shell_env, npm_registries, default_registry, npm_auth)
...
sh_binary(
    ...
    # Look for first-party directory packages
    for snapshot_key, snapshot_info in snapshots.items():
        ...
        if resolution.get("type", None) == "directory":
            ...
            if snapshot_key in fp_links:
                fail("Duplicate fp link key: {}\n\told: {}\n\tnew:".format(snapshot_key, fp_links[snapshot_key], fp_link))

            fp_links[snapshot_key] = fp_link
    ...
    for import_path, importer in importers.items():
        ...
        for dep_name, dep_key in dependencies.items():
            snapshot_info = snapshots.get(dep_key)
            package_info = packages.get(snapshot_info["package"]) if snapshot_info != None else None
            if package_info != None and package_info.get("resolution").get("type", None) == "directory":
                if not dep_key in fp_links.keys():
                    msg = "Expected to file: referenced package {} in first-party links {}".format(dep_key, fp_links.keys())
                    fail(msg)
                if dep_name not in fp_links[dep_key]["link_packages"]:
                    fp_links[dep_key]["link_packages"][dep_name] = []
                fp_links[dep_key]["link_packages"][dep_name].append(link_package)
            elif dep_key.startswith("link:"):
                dep_link = dep_key[len("link:"):]
                dep_path = helpers.link_package(root_package, dep_link)
                if dep_key not in fp_links:
                    transitive_deps = {}
                    raw_deps = {}
...
""".format(
    ...
    for fp_key, fp_link in fp_links.items():
        ...
        npm_link_all_packages_bzl.append(_FP_STORE_TMPL.format(
            ...
            package = fp_package,
            version = fp_version,
            package_store_name = utils.package_store_name(fp_key),
            package_store_root = utils.package_store_root,
        ))
        ...
        for pkg_name, link_packages in fp_link_packages.items():
            npm_link_all_packages_bzl.append(_FP_DIRECT_TMPL.format(
                link_packages = link_packages,
                link_visibility = package_visibility,
                pkg = pkg_name,
                maybe_package_alias = "" if pkg_name == fp_package else """\n            package = "{pkg}",""".format(pkg = pkg_name),
            ...
            if "//visibility:public" in package_visibility:
                add_to_link_targets = """        link_targets.append(":{{}}/{pkg}".format(name))""".format(pkg = pkg_name)
                npm_link_all_packages_bzl.append(add_to_link_targets)
                package_scope = pkg_name[:pkg_name.find("/", 1)] if pkg_name[0] == "@" else None
                if package_scope:
...
def _gen_npm_import(rctx, system_tar, _import, link_workspace):
    ...
    maybe_transitive_closure = ("""
        transitive_closure = %s,""" % starlark_codegen_utils.to_dict_list_attr(_import.transitive_closure, 2)) if _import.transitive_closure and len(_import.transitive_closure) > 0 else ""
    maybe_patch_tool = ("""
    ...
    return _NPM_IMPORT_TMPL.format(
        ...
        name = helpers.to_apparent_repo_name(_import.repo_name),
        package_key = _import.package_key,
