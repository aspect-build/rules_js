...
def generate_repository_files(rctx, name, attr, pnpm_lock_label, importers, packages, patched_dependencies, only_built_dependencies, root_package, default_registry, npm_registries, npm_auth, link_workspace, bzlmod):
    # empty line after bzl docstring since buildifier expects this if this file is vendored in
    generated_by_prefix = "\"\"\"@generated by npm_translate_lock(name = \"{}\", pnpm_lock = \"{}\")\"\"\"\n".format(helpers.to_apparent_repo_name(name), str(pnpm_lock_label))

    npm_imports = helpers.get_npm_imports(importers, packages, patched_dependencies, only_built_dependencies, root_package, name, attr, attr.lifecycle_hooks, attr.lifecycle_hooks_execution_requirements, attr.lifecycle_hooks_use_default_shell_env, npm_registries, default_registry, npm_auth)
...
sh_binary(
    ...
    srcs = ["@aspect_rules_js//npm/private:noop.sh"],
            ...
            "exports_files({})".format(starlark_codegen_utils.to_list_attr([
                ...
                getattr(attr, "repositories_bzl_filename", None),
            ])),
        ],
    }
...
""".format(
            defs_bzl_file = "@{}//:{}".format(name, attr.defs_bzl_filename),
    ...
    if not bzlmod:
        rctx_files[attr.repositories_bzl_filename] = _generate_repositories(
            rctx,
            npm_imports,
            pnpm_lock_label,
            link_workspace,
