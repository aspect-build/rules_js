    ...
    for _import in npm_imports:
        for link_package, link_names in _import.link_packages.items():
            ...
            for link_name in link_names:
                if link_name["pkg"] not in all_public_links[link_package]:
                    all_public_links[link_package][link_name["pkg"]] = []
                all_public_links[link_package][link_name["pkg"]].append("{}@{}".format(_import.package, _import.version))
...
def _get_npm_imports(importers, packages, patched_dependencies, only_built_dependencies, root_package, rctx_name, attr, all_lifecycle_hooks, all_lifecycle_hooks_execution_requirements, all_lifecycle_hooks_use_default_shell_env, registries, default_registry, npm_auth):
    ...
    for import_path, importer in importers.items():
        ...
        for deps_type in ["deps", "dev_deps"]:
            ...
            if type(deps) != "dict":
                fail("expected dict of dependencies in processed importer '{}'".format(import_path))
...
""".format(
        ...
        link_packages = {}
        for _, links in importer_links.items():
            link_names = links["packages"].get(package_key, [])
            if link_names:
