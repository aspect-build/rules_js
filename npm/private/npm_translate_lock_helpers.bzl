...
""".format(
        ...
        c = _dep_constraints(packages, package_key, package_info)

        result_pkg = struct(
            custom_postinstall = custom_postinstall,
            deps = optional_deps | deps,
            deps_os_constraints = c["os"],
            deps_cpu_constraints = c["cpu"],
            integrity = integrity,
            link_packages = link_packages,
            repo_name = repo_name,
...
################################################################################
def _dep_constraints(packages, package_key, package_info):
    os_constraints = {}
    cpu_constraints = {}
    ...
    for name, key in package_info["optional_dependencies"].items():
        ...
        if packages[key]["os"]:
            os_constraints[key] = [pnpm.pnpm_to_bazel_os(os) for os in packages[key]["os"]]
        if packages[key]["cpu"]:
            cpu_constraints[key] = [pnpm.pnpm_to_bazel_cpu(cpu) for cpu in packages[key]["cpu"]]
