...
load("//npm/private:npm_translate_lock.bzl", "npm_translate_lock_lib", _npm_translate_lock = "npm_translate_lock")
...
def _npm_extension_impl(module_ctx):
    for mod in module_ctx.modules:
        for attr in mod.tags.npm_translate_lock:
            ...
            _npm_translate_lock_ext(module_ctx, attr)

        for i in mod.tags.npm_import:
            _npm_import_ext(i)

def _npm_translate_lock_ext(module_ctx, attr):
    _npm_translate_lock(
        name = attr.name,
        bins = attr.bins,
