...
Companion to `npm_translate_lock_auth`. That test authenticates through `.npmrc`; this one
keeps the `.npmrc` free of credentials and uses a
[Bazel credential helper](https://bazel.build/reference/command-line-reference#flag--credential_helper)
for both sides of the private registries:
...
-   `update_pnpm_lock` runs `pnpm install`, which knows nothing about Bazel, so the same helper
    is passed to `npm_translate_lock(credential_helper = ...)` and rules_js hands the credentials
