"""@generated by npm_translate_lock(name = "lock-v61", pnpm_lock = "@//v61:pnpm-lock.yaml")"""

load("@aspect_rules_js//npm:repositories.bzl", "npm_import")

# Generated npm_import repository rules corresponding to npm packages in @//v61:pnpm-lock.yaml
# buildifier: disable=function-docstring
def npm_repositories():
    npm_import(
        name = "lock-v61__ansi-regex__5.0.1",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "ansi-regex",
        version = "5.0.1",
        url = "https://registry.npmjs.org/ansi-regex/-/ansi-regex-5.0.1.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-quJQXlTSUGL2LH9SUXo8VwsY4soanhgo6LNSm84E1LBcE8s3O0wpdiRzyR9z/ZZJMlMWv37qOOb9pdJlMUEKFQ==",
        transitive_closure = {
            "ansi-regex": ["5.0.1"],
        },
    )

    npm_import(
        name = "lock-v61__ansi-regex__6.2.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "ansi-regex",
        version = "6.2.0",
        url = "https://registry.npmjs.org/ansi-regex/-/ansi-regex-6.2.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-TKY5pyBkHyADOPYlRT9Lx6F544mPl0vS5Ew7BJ45hA08Q+t3GjbueLliBWN3sMICk6+y7HdyxSzC4bWS8baBdg==",
        transitive_closure = {
            "ansi-regex": ["6.2.0"],
        },
    )

    npm_import(
        name = "lock-v61__ansi-styles__4.3.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "ansi-styles",
        version = "4.3.0",
        url = "https://registry.npmjs.org/ansi-styles/-/ansi-styles-4.3.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-zbB9rCJAT1rbjiVDb2hqKFHNYLxgtk8NURxZ3IZwD3F6NtxbXZQCnnSi1Lkx+IDohdPlFp222wVALIheZJQSEg==",
        deps = {
            "color-convert": "2.0.1",
        },
        transitive_closure = {
            "ansi-styles": ["4.3.0"],
            "color-convert": ["2.0.1"],
            "color-name": ["1.1.4"],
        },
    )

    npm_import(
        name = "lock-v61__ansi-styles__6.2.1",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "ansi-styles",
        version = "6.2.1",
        url = "https://registry.npmjs.org/ansi-styles/-/ansi-styles-6.2.1.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-bN798gFfQX+viw3R7yrGWRqnrN2oRkEkUjjl4JNn4E8GxxbjtG3FbrEIIY3l8/hrwUwIeCZvi4QuOTP4MErVug==",
        transitive_closure = {
            "ansi-styles": ["6.2.1"],
        },
    )

    npm_import(
        name = "lock-v61__at_aspect-test_a__5.0.2",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["@aspect-test-a-bad-scope", "@aspect-test-custom-scope/a", "@aspect-test/a", "@aspect-test/a2", "aspect-test-a-no-scope", "aspect-test-a/no-at"],
        },
        package = "@aspect-test/a",
        version = "5.0.2",
        url = "https://registry.npmjs.org/@aspect-test/a/-/a-5.0.2.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-bURS+F0+tS2XPxUPbrqsTZxIre1U5ZglwzDqcOCrU7MbxuRrkO24hesgTMGJldCglwL/tiEGRlvdMndlPgRdNw==",
        deps = {
            "@aspect-test/b": "5.0.2",
            "@aspect-test/c": "2.0.2",
            "@aspect-test/d": "2.0.0_at_aspect-test_c_2.0.2",
        },
        transitive_closure = {
            "@aspect-test/a": ["5.0.2"],
            "@aspect-test/b": ["5.0.2"],
            "@aspect-test/c": ["2.0.2"],
            "@aspect-test/d": ["2.0.0_at_aspect-test_c_2.0.2"],
        },
    )

    npm_import(
        name = "lock-v61__at_aspect-test_b__5.0.2",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["@aspect-test/b"],
        },
        package = "@aspect-test/b",
        version = "5.0.2",
        url = "https://registry.npmjs.org/@aspect-test/b/-/b-5.0.2.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-I8wnJV5J0h8ui1O3K6XPq1qGHKopTl/OnvkSfor7uJ9yRCm2Qv6Tf2LsTgR2xzkgiwhA4iBwdYFwecwinF244w==",
        deps = {
            "@aspect-test/a": "5.0.2",
            "@aspect-test/c": "2.0.2",
            "@aspect-test/d": "2.0.0_at_aspect-test_c_2.0.2",
        },
        transitive_closure = {
            "@aspect-test/a": ["5.0.2"],
            "@aspect-test/b": ["5.0.2"],
            "@aspect-test/c": ["2.0.2"],
            "@aspect-test/d": ["2.0.0_at_aspect-test_c_2.0.2"],
        },
    )

    npm_import(
        name = "lock-v61__at_aspect-test_c__2.0.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["@aspect-test/c"],
            "projects/peers-combo-2": ["@aspect-test/c"],
        },
        package = "@aspect-test/c",
        version = "2.0.0",
        url = "https://registry.npmjs.org/@aspect-test/c/-/c-2.0.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-vRuHi/8zxZ+IRGdgdX4VoMNFZrR9UqO87yQx61IGIkjgV7QcKUeu5jfvIE3Mr0WNQeMdO1JpyTx1UUpsE73iug==",
        transitive_closure = {
            "@aspect-test/c": ["2.0.0"],
        },
        lifecycle_hooks = ["preinstall", "install", "postinstall"],
        lifecycle_hooks_execution_requirements = ["no-sandbox"],
    )

    npm_import(
        name = "lock-v61__at_aspect-test_c__2.0.1",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "projects/peers-combo-1": ["@aspect-test/c"],
        },
        package = "@aspect-test/c",
        version = "2.0.1",
        url = "https://registry.npmjs.org/@aspect-test/c/-/c-2.0.1.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-pyetgkZm4yfHYJYFaIi0rXM2VeR9qGw+gukEkrUO7LXuDIfkuvQd5TDduwIYIVvXGRjHKKjCa2BaA153nZfFyQ==",
        transitive_closure = {
            "@aspect-test/c": ["2.0.1"],
        },
        lifecycle_hooks = ["preinstall", "install", "postinstall"],
        lifecycle_hooks_execution_requirements = ["no-sandbox"],
    )

    npm_import(
        name = "lock-v61__at_aspect-test_c__2.0.2",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "@aspect-test/c",
        version = "2.0.2",
        url = "https://registry.npmjs.org/@aspect-test/c/-/c-2.0.2.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-rMJmd3YBvY7y0jh+2m72TiAhe6dVKjMMNFFVOXFCbM233m7lsG4cq970H1C8rUsc3AcA5E/cEHlxSVffHlHD2Q==",
        transitive_closure = {
            "@aspect-test/c": ["2.0.2"],
        },
        lifecycle_hooks = ["preinstall", "install", "postinstall"],
        lifecycle_hooks_execution_requirements = ["no-sandbox"],
    )

    npm_import(
        name = "lock-v61__at_aspect-test_d__2.0.0_at_aspect-test_c_2.0.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "projects/peers-combo-2": ["@aspect-test/d"],
        },
        package = "@aspect-test/d",
        version = "2.0.0_at_aspect-test_c_2.0.0",
        url = "https://registry.npmjs.org/@aspect-test/d/-/d-2.0.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-jndwr8pLUfn795uApTcXG/yZ5hV2At1aS/wo5BVLxqlVVgLoOETF/Dp4QOjMHE/SXkXFowz6Hao+WpmzVvAO0A==",
        deps = {
            "@aspect-test/c": "2.0.0",
        },
        transitive_closure = {
            "@aspect-test/c": ["2.0.0"],
            "@aspect-test/d": ["2.0.0_at_aspect-test_c_2.0.0"],
        },
    )

    npm_import(
        name = "lock-v61__at_aspect-test_d__2.0.0_at_aspect-test_c_2.0.1",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "projects/peers-combo-1": ["@aspect-test/d"],
        },
        package = "@aspect-test/d",
        version = "2.0.0_at_aspect-test_c_2.0.1",
        url = "https://registry.npmjs.org/@aspect-test/d/-/d-2.0.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-jndwr8pLUfn795uApTcXG/yZ5hV2At1aS/wo5BVLxqlVVgLoOETF/Dp4QOjMHE/SXkXFowz6Hao+WpmzVvAO0A==",
        deps = {
            "@aspect-test/c": "2.0.1",
        },
        transitive_closure = {
            "@aspect-test/c": ["2.0.1"],
            "@aspect-test/d": ["2.0.0_at_aspect-test_c_2.0.1"],
        },
    )

    npm_import(
        name = "lock-v61__at_aspect-test_d__2.0.0_at_aspect-test_c_2.0.2",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "@aspect-test/d",
        version = "2.0.0_at_aspect-test_c_2.0.2",
        url = "https://registry.npmjs.org/@aspect-test/d/-/d-2.0.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-jndwr8pLUfn795uApTcXG/yZ5hV2At1aS/wo5BVLxqlVVgLoOETF/Dp4QOjMHE/SXkXFowz6Hao+WpmzVvAO0A==",
        deps = {
            "@aspect-test/c": "2.0.2",
        },
        transitive_closure = {
            "@aspect-test/c": ["2.0.2"],
            "@aspect-test/d": ["2.0.0_at_aspect-test_c_2.0.2"],
        },
    )

    npm_import(
        name = "lock-v61__at_aspect-test_e__1.0.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["@aspect-test/e"],
        },
        package = "@aspect-test/e",
        version = "1.0.0",
        url = "https://registry.npmjs.org/@aspect-test/e/-/e-1.0.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-GyAxHYKN650db+xnimHnL2LPz65ilmQsVhCasWA7drDNQn/rfmPiEVMzjRiS7m46scXIERaBmiJMzYDf0bIUbA==",
        transitive_closure = {
            "@aspect-test/e": ["1.0.0"],
        },
    )

    npm_import(
        name = "lock-v61__at_aspect-test_h__1.0.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["@aspect-test/h-is-only-optional"],
        },
        package = "@aspect-test/h",
        version = "1.0.0",
        url = "https://registry.npmjs.org/@aspect-test/h/-/h-1.0.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-U1LStvh2QPmdQN7rlR0PTZZ1btTTcjiHxVmq5SvTxIRgIaJMCIsxcS5ghrd71H/JIwnJOmhI7BEQN3n6Hq9WSw==",
        transitive_closure = {
            "@aspect-test/h": ["1.0.0"],
        },
    )

    npm_import(
        name = "lock-v61__at_foo_jsonify__at_github.com_aspect-build_test-packages_releases_download_0.0.0_at_foo-jsonify-0.0.0.tgz",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["jsonify"],
            "projects/peer-types": ["jsonify"],
        },
        package = "@foo/jsonify",
        version = "@github.com/aspect-build/test-packages/releases/download/0.0.0/@foo-jsonify-0.0.0.tgz",
        url = "https://github.com/aspect-build/test-packages/releases/download/0.0.0/@foo-jsonify-0.0.0.tgz",
        package_visibility = ["//visibility:public"],
        transitive_closure = {
            "@foo/jsonify": ["@github.com/aspect-build/test-packages/releases/download/0.0.0/@foo-jsonify-0.0.0.tgz"],
        },
    )

    npm_import(
        name = "lock-v61__at_isaacs_cliui__8.0.2",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["@isaacs/cliui"],
        },
        package = "@isaacs/cliui",
        version = "8.0.2",
        url = "https://registry.npmjs.org/@isaacs/cliui/-/cliui-8.0.2.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-O8jcjabXaleOG9DQ0+ARXWZBTfnP4WNAqzuiJK7ll44AmxGKv/J2M4TPjxjY3znBCfvBXFzucm1twdyFybFqEA==",
        deps = {
            "string-width": "5.1.2",
            "string-width-cjs": "npm:string-width@4.2.3",
            "strip-ansi": "7.1.0",
            "strip-ansi-cjs": "npm:strip-ansi@6.0.1",
            "wrap-ansi": "8.1.0",
            "wrap-ansi-cjs": "npm:wrap-ansi@7.0.0",
        },
        transitive_closure = {
            "@isaacs/cliui": ["8.0.2"],
            "ansi-regex": ["5.0.1", "6.2.0"],
            "ansi-styles": ["4.3.0", "6.2.1"],
            "color-convert": ["2.0.1"],
            "color-name": ["1.1.4"],
            "eastasianwidth": ["0.2.0"],
            "emoji-regex": ["8.0.0", "9.2.2"],
            "is-fullwidth-code-point": ["3.0.0"],
            "string-width": ["5.1.2", "4.2.3"],
            "strip-ansi": ["7.1.0", "6.0.1"],
            "wrap-ansi": ["8.1.0", "7.0.0"],
        },
    )

    npm_import(
        name = "lock-v61__at_rollup_plugin-typescript__8.2.1_1813138439",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["rollup-plugin-with-peers"],
        },
        package = "@rollup/plugin-typescript",
        version = "8.2.1_1813138439",
        url = "https://registry.npmjs.org/@rollup/plugin-typescript/-/plugin-typescript-8.2.1.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-Qd2E1pleDR4bwyFxqbjt4eJf+wB0UKVMLc7/BAFDGVdAXQMCsD4DUv5/7/ww47BZCYxWtJqe1Lo0KVNswBJlRw==",
        deps = {
            "@rollup/pluginutils": "3.1.0_rollup_3.29.4",
            "resolve": "1.22.10",
            "rollup": "3.29.4",
            "tslib": "2.6.3",
            "typescript": "5.5.2",
        },
        transitive_closure = {
            "@rollup/plugin-typescript": ["8.2.1_1813138439"],
            "@rollup/pluginutils": ["3.1.0_rollup_3.29.4"],
            "@types/estree": ["0.0.39"],
            "estree-walker": ["1.0.1"],
            "fsevents": ["2.3.3"],
            "function-bind": ["1.1.2"],
            "hasown": ["2.0.2"],
            "is-core-module": ["2.16.1"],
            "path-parse": ["1.0.7"],
            "picomatch": ["2.3.1"],
            "resolve": ["1.22.10"],
            "rollup": ["3.29.4"],
            "supports-preserve-symlinks-flag": ["1.0.0"],
            "tslib": ["2.6.3"],
            "typescript": ["5.5.2"],
        },
    )

    npm_import(
        name = "lock-v61__at_rollup_pluginutils__3.1.0_rollup_3.29.4",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "@rollup/pluginutils",
        version = "3.1.0_rollup_3.29.4",
        url = "https://registry.npmjs.org/@rollup/pluginutils/-/pluginutils-3.1.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-GksZ6pr6TpIjHm8h9lSQ8pi8BE9VeubNT0OMJ3B5uZJ8pz73NPiqOtCog/x2/QzM1ENChPKxMDhiQuRHsqc+lg==",
        deps = {
            "@types/estree": "0.0.39",
            "estree-walker": "1.0.1",
            "picomatch": "2.3.1",
            "rollup": "3.29.4",
        },
        transitive_closure = {
            "@rollup/pluginutils": ["3.1.0_rollup_3.29.4"],
            "@types/estree": ["0.0.39"],
            "estree-walker": ["1.0.1"],
            "fsevents": ["2.3.3"],
            "picomatch": ["2.3.1"],
            "rollup": ["3.29.4"],
        },
    )

    npm_import(
        name = "lock-v61__at_types_archiver__5.3.1",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["@types/archiver"],
        },
        package = "@types/archiver",
        version = "5.3.1",
        url = "https://registry.yarnpkg.com/@types/archiver/-/archiver-5.3.1.tgz",
        package_visibility = ["//visibility:public"],
        dev = True,
        integrity = "sha512-wKYZaSXaDvTZuInAWjCeGG7BEAgTWG2zZW0/f7IYFcoHB2X2d9lkVFnrOlXl3W6NrvO6Ml3FLLu8Uksyymcpnw==",
        deps = {
            "@types/glob": "8.1.0",
        },
        transitive_closure = {
            "@types/archiver": ["5.3.1"],
            "@types/glob": ["8.1.0"],
            "@types/minimatch": ["5.1.2"],
            "@types/node": ["18.19.54"],
            "undici-types": ["5.26.5"],
        },
    )

    npm_import(
        name = "lock-v61__at_types_estree__0.0.39",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "@types/estree",
        version = "0.0.39",
        url = "https://registry.yarnpkg.com/@types/estree/-/estree-0.0.39.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-EYNwp3bU+98cpU4lAWYYL7Zz+2gryWH1qbdDTidVd6hkiR6weksdbMadyXKXNPEkQFhXM+hVO9ZygomHXp+AIw==",
        transitive_closure = {
            "@types/estree": ["0.0.39"],
        },
    )

    npm_import(
        name = "lock-v61__at_types_glob__8.1.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "@types/glob",
        version = "8.1.0",
        url = "https://registry.yarnpkg.com/@types/glob/-/glob-8.1.0.tgz",
        package_visibility = ["//visibility:public"],
        dev = True,
        integrity = "sha512-IO+MJPVhoqz+28h1qLAcBEH2+xHMK6MTyHJc7MTnnYb6wsoLR29POVGJ7LycmVXIqyy/4/2ShP5sUwTXuOwb/w==",
        deps = {
            "@types/minimatch": "5.1.2",
            "@types/node": "18.19.54",
        },
        transitive_closure = {
            "@types/glob": ["8.1.0"],
            "@types/minimatch": ["5.1.2"],
            "@types/node": ["18.19.54"],
            "undici-types": ["5.26.5"],
        },
    )

    npm_import(
        name = "lock-v61__at_types_minimatch__5.1.2",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "@types/minimatch",
        version = "5.1.2",
        url = "https://registry.yarnpkg.com/@types/minimatch/-/minimatch-5.1.2.tgz",
        package_visibility = ["//visibility:public"],
        dev = True,
        integrity = "sha512-K0VQKziLUWkVKiRVrx4a40iPaxTUefQmjtkQofBkYRcoaaL/8rhwDWww9qWbrgicNOgnpIsMxyNIUM4+n6dUIA==",
        transitive_closure = {
            "@types/minimatch": ["5.1.2"],
        },
    )

    npm_import(
        name = "lock-v61__at_types_node__18.19.54",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["@types/node", "alias-types-node"],
            "projects/a-types": ["@types/node"],
            "projects/b": ["@types/node"],
        },
        package = "@types/node",
        version = "18.19.54",
        url = "https://registry.yarnpkg.com/@types/node/-/node-18.19.54.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-+BRgt0G5gYjTvdLac9sIeE0iZcJxi4Jc4PV5EUzqi+88jmQLr+fRZdv2tCTV7IHKSGxM6SaLoOXQWWUiLUItMw==",
        deps = {
            "undici-types": "5.26.5",
        },
        transitive_closure = {
            "@types/node": ["18.19.54"],
            "undici-types": ["5.26.5"],
        },
    )

    npm_import(
        name = "lock-v61__at_types_sizzle__2.3.10",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["alias-only-sizzle"],
        },
        package = "@types/sizzle",
        version = "2.3.10",
        url = "https://registry.yarnpkg.com/@types/sizzle/-/sizzle-2.3.10.tgz",
        package_visibility = ["//visibility:public"],
        dev = True,
        integrity = "sha512-TC0dmN0K8YcWEAEfiPi5gJP14eJe30TTGjkvek3iM/1NdHHsdCA/Td6GvNndMOo/iSnIsZ4HuuhrYPDAmbxzww==",
        transitive_closure = {
            "@types/sizzle": ["2.3.10"],
        },
    )

    npm_import(
        name = "lock-v61__color-convert__2.0.1",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "color-convert",
        version = "2.0.1",
        url = "https://registry.npmjs.org/color-convert/-/color-convert-2.0.1.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-RRECPsj7iu/xb5oKYcsFHSppFNnsj/52OVTRKb4zP5onXwVF3zVmmToNcOfGC+CRDpfK/U584fMg38ZHCaElKQ==",
        deps = {
            "color-name": "1.1.4",
        },
        transitive_closure = {
            "color-convert": ["2.0.1"],
            "color-name": ["1.1.4"],
        },
    )

    npm_import(
        name = "lock-v61__color-name__1.1.4",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "color-name",
        version = "1.1.4",
        url = "https://registry.npmjs.org/color-name/-/color-name-1.1.4.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-dOy+3AuW3a2wNbZHIuMZpTcgjGuLU/uBL/ubcZF9OXbDo8ff4O8yVp5Bf0efS8uEoYo5q4Fx7dY9OgQGXgAsQA==",
        transitive_closure = {
            "color-name": ["1.1.4"],
        },
    )

    npm_import(
        name = "lock-v61__debug__github.com_ngokevin_debug_9742c5f383a6f8046241920156236ade8ec30d53",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["debug"],
        },
        package = "debug",
        version = "github.com/ngokevin/debug/9742c5f383a6f8046241920156236ade8ec30d53",
        url = "https://codeload.github.com/ngokevin/debug/tar.gz/9742c5f383a6f8046241920156236ade8ec30d53",
        package_visibility = ["//visibility:public"],
        deps = {
            "ms": "0.7.3",
        },
        transitive_closure = {
            "debug": ["github.com/ngokevin/debug/9742c5f383a6f8046241920156236ade8ec30d53"],
            "ms": ["0.7.3"],
        },
    )

    npm_import(
        name = "lock-v61__dequal__2.0.3",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "dequal",
        version = "2.0.3",
        url = "https://registry.npmjs.org/dequal/-/dequal-2.0.3.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-0je+qPKHEMohvfRTCEo3CrPG6cAzAYgmzKyxRiYSSDkS6eGJdyVJm7WaYA5ECaAD9wLB2T4EEeymA5aFVcYXCA==",
        transitive_closure = {
            "dequal": ["2.0.3"],
        },
    )

    npm_import(
        name = "lock-v61__diff__at_github.com_kpdecker_jsdiff_archive_refs_tags_v5.2.0.tar.gz",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "diff",
        version = "@github.com/kpdecker/jsdiff/archive/refs/tags/v5.2.0.tar.gz",
        url = "https://github.com/kpdecker/jsdiff/archive/refs/tags/v5.2.0.tar.gz",
        package_visibility = ["//visibility:public"],
        transitive_closure = {
            "diff": ["@github.com/kpdecker/jsdiff/archive/refs/tags/v5.2.0.tar.gz"],
        },
    )

    npm_import(
        name = "lock-v61__eastasianwidth__0.2.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "eastasianwidth",
        version = "0.2.0",
        url = "https://registry.npmjs.org/eastasianwidth/-/eastasianwidth-0.2.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-I88TYZWc9XiYHRQ4/3c5rjjfgkjhLyW2luGIheGERbNQ6OY7yTybanSpDXZa8y7VUP9YmDcYa+eyq4ca7iLqWA==",
        transitive_closure = {
            "eastasianwidth": ["0.2.0"],
        },
    )

    npm_import(
        name = "lock-v61__emoji-regex__8.0.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "emoji-regex",
        version = "8.0.0",
        url = "https://registry.npmjs.org/emoji-regex/-/emoji-regex-8.0.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-MSjYzcWNOA0ewAHpz0MxpYFvwg6yjy1NG3xteoqz644VCo/RPgnr1/GGt+ic3iJTzQ8Eu3TdM14SawnVUmGE6A==",
        transitive_closure = {
            "emoji-regex": ["8.0.0"],
        },
    )

    npm_import(
        name = "lock-v61__emoji-regex__9.2.2",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "emoji-regex",
        version = "9.2.2",
        url = "https://registry.npmjs.org/emoji-regex/-/emoji-regex-9.2.2.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-L18DaJsXSUk2+42pv8mLs5jJT2hqFkFE4j21wOmgbUqsZ2hL72NsUU785g9RXgo3s0ZNgVl42TiHp3ZtOv/Vyg==",
        transitive_closure = {
            "emoji-regex": ["9.2.2"],
        },
    )

    npm_import(
        name = "lock-v61__estree-walker__1.0.1",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "estree-walker",
        version = "1.0.1",
        url = "https://registry.npmjs.org/estree-walker/-/estree-walker-1.0.1.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-1fMXF3YP4pZZVozF8j/ZLfvnR8NSIljt56UhbZ5PeeDmmGHpgpdwQt7ITlGvYaQukCvuBRMLEiKiYC+oeIg4cg==",
        transitive_closure = {
            "estree-walker": ["1.0.1"],
        },
    )

    npm_import(
        name = "lock-v61__fsevents__2.3.3",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "fsevents",
        version = "2.3.3",
        url = "https://registry.npmjs.org/fsevents/-/fsevents-2.3.3.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-5xoDfX+fL7faATnagmWPpbFtwh/R77WmMMqqHGS65C3vvB0YHrgF+B1YmZ3441tMj5n63k0212XNoJwzlhffQw==",
        transitive_closure = {
            "fsevents": ["2.3.3"],
        },
    )

    npm_import(
        name = "lock-v61__function-bind__1.1.2",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "function-bind",
        version = "1.1.2",
        url = "https://registry.npmjs.org/function-bind/-/function-bind-1.1.2.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-7XHNxH7qX9xG5mIwxkhumTox/MIRNcOgDrxWsMt2pAr23WHp6MrRlN7FBSFpCpr+oVO0F744iUgR82nJMfG2SA==",
        transitive_closure = {
            "function-bind": ["1.1.2"],
        },
    )

    npm_import(
        name = "lock-v61__hasown__2.0.2",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "hasown",
        version = "2.0.2",
        url = "https://registry.npmjs.org/hasown/-/hasown-2.0.2.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-0hJU9SCPvmMzIBdZFqNPXWa6dqh7WdH0cII9y+CyS8rG3nL48Bclra9HmKhVVUHyPWNH5Y7xDwAB7bfgSjkUMQ==",
        deps = {
            "function-bind": "1.1.2",
        },
        transitive_closure = {
            "function-bind": ["1.1.2"],
            "hasown": ["2.0.2"],
        },
    )

    npm_import(
        name = "lock-v61__hello__at_gitpkg.vercel.app_EqualMa_gitpkg-hello_packages_hello",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["hello"],
            "projects/peer-types": ["hello"],
        },
        package = "hello",
        version = "@gitpkg.vercel.app/EqualMa/gitpkg-hello/packages/hello",
        url = "https://gitpkg.vercel.app/EqualMa/gitpkg-hello/packages/hello",
        package_visibility = ["//visibility:public"],
        transitive_closure = {
            "hello": ["@gitpkg.vercel.app/EqualMa/gitpkg-hello/packages/hello"],
        },
    )

    npm_import(
        name = "lock-v61__is-core-module__2.16.1",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "is-core-module",
        version = "2.16.1",
        url = "https://registry.npmjs.org/is-core-module/-/is-core-module-2.16.1.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-UfoeMA6fIJ8wTYFEUjelnaGI67v6+N7qXJEvQuIGa99l4xsCruSYOVSQ0uPANn4dAzm8lkYPaKLrrijLq7x23w==",
        deps = {
            "hasown": "2.0.2",
        },
        transitive_closure = {
            "function-bind": ["1.1.2"],
            "hasown": ["2.0.2"],
            "is-core-module": ["2.16.1"],
        },
    )

    npm_import(
        name = "lock-v61__is-fullwidth-code-point__3.0.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "is-fullwidth-code-point",
        version = "3.0.0",
        url = "https://registry.npmjs.org/is-fullwidth-code-point/-/is-fullwidth-code-point-3.0.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-zymm5+u+sCsSWyD9qNaejV3DFvhCKclKdizYaJUuHA83RLjb7nSuGnddCHGv0hk+KY7BMAlsWeK4Ueg6EV6XQg==",
        transitive_closure = {
            "is-fullwidth-code-point": ["3.0.0"],
        },
    )

    npm_import(
        name = "lock-v61__is-odd__0.1.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["is-odd-v0"],
        },
        package = "is-odd",
        version = "0.1.0",
        url = "https://registry.npmjs.org/is-odd/-/is-odd-0.1.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-3RKAybrJxq3zCUC+TJ5Ao0sBsbacAT3OBeNVcCbsQsHUC70qWK2R4JsIvax4OTjeGWnB8FumAWATUtS1jd+KYw==",
        deps = {
            "is-number": "file:../vendored/is-number",
        },
        transitive_closure = {
            "is-number": ["file:../vendored/is-number"],
            "is-odd": ["0.1.0"],
        },
    )

    npm_import(
        name = "lock-v61__is-odd__1.0.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["is-odd-v1"],
        },
        package = "is-odd",
        version = "1.0.0",
        url = "https://registry.npmjs.org/is-odd/-/is-odd-1.0.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-yIhxkKCK7pZnj48WBvaTQ36Or7PymGYqmZrSNqkrhmU3pakkp2TWhuN7hH25bqJgH+Xq4IZ3QNd3+QVshldEHA==",
        deps = {
            "is-number": "file:../vendored/is-number",
        },
        transitive_closure = {
            "is-number": ["file:../vendored/is-number"],
            "is-odd": ["1.0.0"],
        },
    )

    npm_import(
        name = "lock-v61__is-odd__2.0.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["is-odd-v2"],
        },
        package = "is-odd",
        version = "2.0.0",
        url = "https://registry.npmjs.org/is-odd/-/is-odd-2.0.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-OTiixgpZAT1M4NHgS5IguFp/Vz2VI3U7Goh4/HA1adtwyLtSBrxYlcSYkhpAE07s4fKEcjrFxyvtQBND4vFQyQ==",
        deps = {
            "is-number": "file:../vendored/is-number",
        },
        transitive_closure = {
            "is-number": ["file:../vendored/is-number"],
            "is-odd": ["2.0.0"],
        },
    )

    npm_import(
        name = "lock-v61__is-odd__3.0.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["is-odd-v3"],
        },
        package = "is-odd",
        version = "3.0.0",
        url = "https://registry.npmjs.org/is-odd/-/is-odd-3.0.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-204vE5IJ0Cd6pA6x1dMyLooGk6/xeKuq90imFuJN/ndMDBP4Sk9tJpBlTedHPvt6KDbtTDTsjVzzgctFqNV7FQ==",
        deps = {
            "is-number": "file:../vendored/is-number",
        },
        transitive_closure = {
            "is-number": ["file:../vendored/is-number"],
            "is-odd": ["3.0.0"],
        },
    )

    npm_import(
        name = "lock-v61__is-odd__3.0.1",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["is-odd", "is-odd-alias"],
        },
        package = "is-odd",
        version = "3.0.1",
        url = "https://registry.npmjs.org/is-odd/-/is-odd-3.0.1.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-CQpnWPrDwmP1+SMHXZhtLtJv90yiyVfluGsX5iNCVkrhQtU3TQHsUWPG9wkdk9Lgd5yNpAg9jQEo90CBaXgWMA==",
        deps = {
            "is-number": "file:../vendored/is-number",
        },
        transitive_closure = {
            "is-number": ["file:../vendored/is-number"],
            "is-odd": ["3.0.1"],
        },
    )

    npm_import(
        name = "lock-v61__jquery__github.com_jquery_jquery_399b201bb3143a3952894cf3489b4848fc003967",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["jquery-git-ssh-399b201"],
        },
        package = "jquery",
        version = "github.com/jquery/jquery/399b201bb3143a3952894cf3489b4848fc003967",
        url = "https://codeload.github.com/jquery/jquery/tar.gz/399b201bb3143a3952894cf3489b4848fc003967",
        package_visibility = ["//visibility:public"],
        transitive_closure = {
            "jquery": ["github.com/jquery/jquery/399b201bb3143a3952894cf3489b4848fc003967"],
        },
    )

    npm_import(
        name = "lock-v61__jquery__github.com_jquery_jquery_e61fccb9d736235b4b011f89cba6866bc0b8997d",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["jquery-git-ssh-e61fccb"],
        },
        package = "jquery",
        version = "github.com/jquery/jquery/e61fccb9d736235b4b011f89cba6866bc0b8997d",
        url = "https://codeload.github.com/jquery/jquery/tar.gz/e61fccb9d736235b4b011f89cba6866bc0b8997d",
        package_visibility = ["//visibility:public"],
        transitive_closure = {
            "jquery": ["github.com/jquery/jquery/e61fccb9d736235b4b011f89cba6866bc0b8997d"],
        },
    )

    npm_import(
        name = "lock-v61__kleur__4.1.5",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "kleur",
        version = "4.1.5",
        url = "https://registry.npmjs.org/kleur/-/kleur-4.1.5.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-o+NO+8WrRiQEE4/7nwRJhN1HWpVmJm511pBHUxPLtp0BUISzlBplORYSmTclCnJvQq2tKu/sgl3xVpkc7ZWuQQ==",
        transitive_closure = {
            "kleur": ["4.1.5"],
        },
    )

    npm_import(
        name = "lock-v61__lodash__4.17.21",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "projects/alts": ["lodash-dupe"],
        },
        package = "lodash",
        version = "4.17.21",
        url = "https://registry.npmjs.org/lodash/-/lodash-4.17.21.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-v2kDEe57lecTulaDIuNTPy3Ry4gLGJ6Z1O3vE1krgXZNrsQ+LFTGHVxVjcXPs17LhbZVGedAJv8XZ1tvj5FvSg==",
        transitive_closure = {
            "lodash": ["4.17.21"],
        },
    )

    npm_import(
        name = "lock-v61__lodash__file_.._vendored_lodash-4.17.21.tgz",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["lodash"],
            "projects/alts": ["lodash", "lodash-file"],
        },
        package = "lodash",
        version = "file:../vendored/lodash-4.17.21.tgz",
        url = "file:vendored/lodash-4.17.21.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-v2kDEe57lecTulaDIuNTPy3Ry4gLGJ6Z1O3vE1krgXZNrsQ+LFTGHVxVjcXPs17LhbZVGedAJv8XZ1tvj5FvSg==",
        transitive_closure = {
            "lodash": ["file:../vendored/lodash-4.17.21.tgz"],
        },
    )

    npm_import(
        name = "lock-v61__meaning-of-life__1.0.0_o3deharooos255qt5xdujc3cuq",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["meaning-of-life"],
        },
        package = "meaning-of-life",
        version = "1.0.0_o3deharooos255qt5xdujc3cuq",
        url = "https://registry.npmjs.org/meaning-of-life/-/meaning-of-life-1.0.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-fVA4xSydqtK9owabGcYw1r4EKEsMOVVeYQLeCXPu77Z+8Y2j2B2I16UqZlKIOHnYkJ4RSvpJ00ywy9IWjmuxYw==",
        transitive_closure = {
            "meaning-of-life": ["1.0.0_o3deharooos255qt5xdujc3cuq"],
        },
        patches = ["@//v61:patches/meaning-of-life@1.0.0-pnpm.patch"],
        patch_args = ["-p1"],
    )

    npm_import(
        name = "lock-v61__mri__1.2.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "mri",
        version = "1.2.0",
        url = "https://registry.npmjs.org/mri/-/mri-1.2.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-tzzskb3bG8LvYGFF/mDTpq3jpI6Q9wc3LEmBaghu+DdCssd1FakN7Bc0hVNmEyGq1bq3RgfkCb3cmQLpNPOroA==",
        transitive_closure = {
            "mri": ["1.2.0"],
        },
    )

    npm_import(
        name = "lock-v61__ms__0.7.3",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "ms",
        version = "0.7.3",
        url = "https://registry.npmjs.org/ms/-/ms-0.7.3.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-lrKNzMWqQZgwJahtrtrM+9NgOoDUveDrVmm5aGXrf3BdtL0mq7X6IVzoZaw+TfNti29eHd1/8GI+h45K5cQ6/w==",
        transitive_closure = {
            "ms": ["0.7.3"],
        },
    )

    npm_import(
        name = "lock-v61__path-parse__1.0.7",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "path-parse",
        version = "1.0.7",
        url = "https://registry.npmjs.org/path-parse/-/path-parse-1.0.7.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-LDJzPVEEEPR+y48z93A0Ed0yXb8pAByGWo/k5YYdYgpY2/2EsOsksJrq7lOHxryrVOn1ejG6oAp8ahvOIQD8sw==",
        transitive_closure = {
            "path-parse": ["1.0.7"],
        },
    )

    npm_import(
        name = "lock-v61__picomatch__2.3.1",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "picomatch",
        version = "2.3.1",
        url = "https://registry.npmjs.org/picomatch/-/picomatch-2.3.1.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-JU3teHTNjmE2VCGFzuY8EXzCDVwEqB2a8fsIvwaStHhAWJEeVd1o1QD80CU6+ZdEXXSLbSsuLwJjkCBWqRQUVA==",
        transitive_closure = {
            "picomatch": ["2.3.1"],
        },
    )

    npm_import(
        name = "lock-v61__resolve__1.22.10",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "resolve",
        version = "1.22.10",
        url = "https://registry.npmjs.org/resolve/-/resolve-1.22.10.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-NPRy+/ncIMeDlTAsuqwKIiferiawhefFJtkNSW0qZJEqMEb+qBt/77B/jGeeek+F0uOeN05CDa6HXbbIgtVX4w==",
        deps = {
            "is-core-module": "2.16.1",
            "path-parse": "1.0.7",
            "supports-preserve-symlinks-flag": "1.0.0",
        },
        transitive_closure = {
            "function-bind": ["1.1.2"],
            "hasown": ["2.0.2"],
            "is-core-module": ["2.16.1"],
            "path-parse": ["1.0.7"],
            "resolve": ["1.22.10"],
            "supports-preserve-symlinks-flag": ["1.0.0"],
        },
    )

    npm_import(
        name = "lock-v61__rollup__2.14.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["rollup"],
        },
        package = "rollup",
        version = "2.14.0",
        url = "https://registry.npmjs.org/rollup/-/rollup-2.14.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-SUsFh2bBemQqCXOCBWRayjK3/6kNHVR8PbSKKYWIdI6e4zuGSW5B1hGVkFi40805dUrqosMLLxMuEyJMylC9YA==",
        deps = {
            "fsevents": "2.3.3",
        },
        transitive_closure = {
            "fsevents": ["2.3.3"],
            "rollup": ["2.14.0"],
        },
    )

    npm_import(
        name = "lock-v61__rollup__3.29.4",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["rollup3"],
        },
        package = "rollup",
        version = "3.29.4",
        url = "https://registry.npmjs.org/rollup/-/rollup-3.29.4.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-oWzmBZwvYrU0iJHtDmhsm662rC15FRXmcjCk1xD771dFDx5jJ02ufAQQTn0etB2emNk4J9EZg/yWKpsn9BWGRw==",
        deps = {
            "fsevents": "2.3.3",
        },
        transitive_closure = {
            "fsevents": ["2.3.3"],
            "rollup": ["3.29.4"],
        },
    )

    npm_import(
        name = "lock-v61__sade__1.8.1",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "sade",
        version = "1.8.1",
        url = "https://registry.npmjs.org/sade/-/sade-1.8.1.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-xal3CZX1Xlo/k4ApwCFrHVACi9fBqJ7V+mwhBsuf/1IOKbBy098Fex+Wa/5QMubw09pSZ/u8EY8PWgevJsXp1A==",
        deps = {
            "mri": "1.2.0",
        },
        transitive_closure = {
            "mri": ["1.2.0"],
            "sade": ["1.8.1"],
        },
    )

    npm_import(
        name = "lock-v61__string-width__4.2.3",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "string-width",
        version = "4.2.3",
        url = "https://registry.npmjs.org/string-width/-/string-width-4.2.3.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-wKyQRQpjJ0sIp62ErSZdGsjMJWsap5oRNihHhu6G7JVO/9jIB6UyevL+tXuOqrng8j/cxKTWyWUwvSTriiZz/g==",
        deps = {
            "emoji-regex": "8.0.0",
            "is-fullwidth-code-point": "3.0.0",
            "strip-ansi": "6.0.1",
        },
        transitive_closure = {
            "ansi-regex": ["5.0.1"],
            "emoji-regex": ["8.0.0"],
            "is-fullwidth-code-point": ["3.0.0"],
            "string-width": ["4.2.3"],
            "strip-ansi": ["6.0.1"],
        },
    )

    npm_import(
        name = "lock-v61__string-width__5.1.2",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "string-width",
        version = "5.1.2",
        url = "https://registry.npmjs.org/string-width/-/string-width-5.1.2.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-HnLOCR3vjcY8beoNLtcjZ5/nxn2afmME6lhrDrebokqMap+XbeW8n9TXpPDOqdGK5qcI3oT0GKTW6wC7EMiVqA==",
        deps = {
            "eastasianwidth": "0.2.0",
            "emoji-regex": "9.2.2",
            "strip-ansi": "7.1.0",
        },
        transitive_closure = {
            "ansi-regex": ["6.2.0"],
            "eastasianwidth": ["0.2.0"],
            "emoji-regex": ["9.2.2"],
            "string-width": ["5.1.2"],
            "strip-ansi": ["7.1.0"],
        },
    )

    npm_import(
        name = "lock-v61__strip-ansi__6.0.1",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "strip-ansi",
        version = "6.0.1",
        url = "https://registry.npmjs.org/strip-ansi/-/strip-ansi-6.0.1.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-Y38VPSHcqkFrCpFnQ9vuSXmquuv5oXOKpGeT6aGrr3o3Gc9AlVa6JBfUSOCnbxGGZF+/0ooI7KrPuUSztUdU5A==",
        deps = {
            "ansi-regex": "5.0.1",
        },
        transitive_closure = {
            "ansi-regex": ["5.0.1"],
            "strip-ansi": ["6.0.1"],
        },
    )

    npm_import(
        name = "lock-v61__strip-ansi__7.1.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "strip-ansi",
        version = "7.1.0",
        url = "https://registry.npmjs.org/strip-ansi/-/strip-ansi-7.1.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-iq6eVVI64nQQTRYq2KtEg2d2uU7LElhTJwsH4YzIHZshxlgZms/wIc4VoDQTlG/IvVIrBKG06CrZnp0qv7hkcQ==",
        deps = {
            "ansi-regex": "6.2.0",
        },
        transitive_closure = {
            "ansi-regex": ["6.2.0"],
            "strip-ansi": ["7.1.0"],
        },
    )

    npm_import(
        name = "lock-v61__supports-preserve-symlinks-flag__1.0.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "supports-preserve-symlinks-flag",
        version = "1.0.0",
        url = "https://registry.npmjs.org/supports-preserve-symlinks-flag/-/supports-preserve-symlinks-flag-1.0.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-ot0WnXS9fgdkgIcePe6RHNk1WA8+muPa6cSjeR3V8K27q9BB1rTE3R1p7Hv0z1ZyAc8s6Vvv8DIyWf681MAt0w==",
        transitive_closure = {
            "supports-preserve-symlinks-flag": ["1.0.0"],
        },
    )

    npm_import(
        name = "lock-v61__tslib__2.6.3",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["tslib"],
        },
        package = "tslib",
        version = "2.6.3",
        url = "https://registry.npmjs.org/tslib/-/tslib-2.6.3.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-xNvxJEOUiWPGhUuUdQgAJPKOOJfGnIyKySOc09XkKsgdUV/3E2zvwZYdejjmRgPCgcym1juLH3226yA7sEFJKQ==",
        transitive_closure = {
            "tslib": ["2.6.3"],
        },
    )

    npm_import(
        name = "lock-v61__typescript__5.5.2",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["typescript"],
        },
        package = "typescript",
        version = "5.5.2",
        url = "https://registry.npmjs.org/typescript/-/typescript-5.5.2.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-NcRtPEOsPFFWjobJEtfihkLCZCXZt/os3zf8nTxjVH3RvTSxjrCamJpbExGvYOF+tFHc3pA65qpdwPbzjohhew==",
        transitive_closure = {
            "typescript": ["5.5.2"],
        },
    )

    npm_import(
        name = "lock-v61__undici-types__5.26.5",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "undici-types",
        version = "5.26.5",
        url = "https://registry.npmjs.org/undici-types/-/undici-types-5.26.5.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-JlCMO+ehdEIKqlFxk6IfVoAUVmgz7cU7zD/h9XZ0qzeosSHmUJVOzSQvvYSYWXkFXC+IfLKSIffhv0sVZup6pA==",
        transitive_closure = {
            "undici-types": ["5.26.5"],
        },
    )

    npm_import(
        name = "lock-v61__uvu__0.5.6",
        root_package = "v61",
        link_workspace = "",
        link_packages = {
            "v61": ["uvu"],
        },
        package = "uvu",
        version = "0.5.6",
        url = "https://registry.npmjs.org/uvu/-/uvu-0.5.6.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-+g8ENReyr8YsOc6fv/NVJs2vFdHBnBNdfE49rshrTzDWOlUx4Gq7KOS2GD8eqhy2j+Ejq29+SbKH8yjkAqXqoA==",
        deps = {
            "dequal": "2.0.3",
            "diff": "@github.com/kpdecker/jsdiff/archive/refs/tags/v5.2.0.tar.gz",
            "kleur": "4.1.5",
            "sade": "1.8.1",
        },
        transitive_closure = {
            "dequal": ["2.0.3"],
            "diff": ["@github.com/kpdecker/jsdiff/archive/refs/tags/v5.2.0.tar.gz"],
            "kleur": ["4.1.5"],
            "mri": ["1.2.0"],
            "sade": ["1.8.1"],
            "uvu": ["0.5.6"],
        },
    )

    npm_import(
        name = "lock-v61__wrap-ansi__7.0.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "wrap-ansi",
        version = "7.0.0",
        url = "https://registry.npmjs.org/wrap-ansi/-/wrap-ansi-7.0.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-YVGIj2kamLSTxw6NsZjoBxfSwsn0ycdesmc4p+Q21c5zPuZ1pl+NfxVdxPtdHvmNVOQ6XSYG4AUtyt/Fi7D16Q==",
        deps = {
            "ansi-styles": "4.3.0",
            "string-width": "4.2.3",
            "strip-ansi": "6.0.1",
        },
        transitive_closure = {
            "ansi-regex": ["5.0.1"],
            "ansi-styles": ["4.3.0"],
            "color-convert": ["2.0.1"],
            "color-name": ["1.1.4"],
            "emoji-regex": ["8.0.0"],
            "is-fullwidth-code-point": ["3.0.0"],
            "string-width": ["4.2.3"],
            "strip-ansi": ["6.0.1"],
            "wrap-ansi": ["7.0.0"],
        },
    )

    npm_import(
        name = "lock-v61__wrap-ansi__8.1.0",
        root_package = "v61",
        link_workspace = "",
        link_packages = {},
        package = "wrap-ansi",
        version = "8.1.0",
        url = "https://registry.npmjs.org/wrap-ansi/-/wrap-ansi-8.1.0.tgz",
        package_visibility = ["//visibility:public"],
        integrity = "sha512-si7QWI6zUMq56bESFvagtmzMdGOtoxfR+Sez11Mobfc7tm+VkUckk9bW2UeffTGVUbOksxmSw0AA2gs8g71NCQ==",
        deps = {
            "ansi-styles": "6.2.1",
            "string-width": "5.1.2",
            "strip-ansi": "7.1.0",
        },
        transitive_closure = {
            "ansi-regex": ["6.2.0"],
            "ansi-styles": ["6.2.1"],
            "eastasianwidth": ["0.2.0"],
            "emoji-regex": ["9.2.2"],
            "string-width": ["5.1.2"],
            "strip-ansi": ["7.1.0"],
            "wrap-ansi": ["8.1.0"],
        },
    )
