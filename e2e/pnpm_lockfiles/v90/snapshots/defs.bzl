    ...
    if link:
        if bazel_package == "<LOCKVERSION>":
            ...
            if "@aspect-test" not in scope_targets:
                ...
                scope_targets["@aspect-test"].append(link_targets[-1])
            link_9("{}/@aspect-test/e".format(name), link_root_name = name, link_alias = "@aspect-test/e")
            link_targets.append(":{}/@aspect-test/e".format(name))
            if "@aspect-test" not in scope_targets:
                scope_targets["@aspect-test"] = [link_targets[-1]]
            else:
                scope_targets["@aspect-test"].append(link_targets[-1])
            link_10("{}/jsonify".format(name), link_root_name = name, link_alias = "jsonify")
            link_targets.append(":{}/jsonify".format(name))
            link_11("{}/@isaacs/cliui".format(name), link_root_name = name, link_alias = "@isaacs/cliui")
            ...
            link_12("{}/rollup-plugin-with-peers".format(name), link_root_name = name, link_alias = "rollup-plugin-with-peers")
            link_targets.append(":{}/rollup-plugin-with-peers".format(name))
            link_14("{}/@types/archiver".format(name), link_root_name = name, link_alias = "@types/archiver")
            ...
            link_18("{}/@types/node".format(name), link_root_name = name, link_alias = "@types/node")
            link_targets.append(":{}/@types/node".format(name))
            if "@types" not in scope_targets:
                scope_targets["@types"] = [link_targets[-1]]
            else:
                scope_targets["@types"].append(link_targets[-1])
            link_18("{}/alias-types-node".format(name), link_root_name = name, link_alias = "alias-types-node")
            link_targets.append(":{}/alias-types-node".format(name))
            link_19("{}/alias-only-sizzle".format(name), link_root_name = name, link_alias = "alias-only-sizzle")
            link_targets.append(":{}/alias-only-sizzle".format(name))
            link_22("{}/debug".format(name), link_root_name = name, link_alias = "debug")
            link_targets.append(":{}/debug".format(name))
            link_32("{}/hello".format(name), link_root_name = name, link_alias = "hello")
            ...
            link_40("{}/jquery-git-ssh-e61fccb".format(name), link_root_name = name, link_alias = "jquery-git-ssh-e61fccb")
            link_targets.append(":{}/jquery-git-ssh-e61fccb".format(name))
            link_42("{}/lodash".format(name), link_root_name = name, link_alias = "lodash")
            link_targets.append(":{}/lodash".format(name))
            link_43("{}/meaning-of-life".format(name), link_root_name = name, link_alias = "meaning-of-life")
            link_targets.append(":{}/meaning-of-life".format(name))
            link_49("{}/rollup".format(name), link_root_name = name, link_alias = "rollup")
            link_targets.append(":{}/rollup".format(name))
            link_50("{}/rollup3".format(name), link_root_name = name, link_alias = "rollup3")
            link_targets.append(":{}/rollup3".format(name))
            link_57("{}/tslib".format(name), link_root_name = name, link_alias = "tslib")
            link_targets.append(":{}/tslib".format(name))
            link_58("{}/typescript".format(name), link_root_name = name, link_alias = "typescript")
            link_targets.append(":{}/typescript".format(name))
            link_59("{}/uvu".format(name), link_root_name = name, link_alias = "uvu")
