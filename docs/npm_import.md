...
| <a id="npm_import-exclude_patterns"></a>exclude_patterns |  List of glob patterns to exclude from the linked package.<br><br>This is useful for excluding files that are not needed in the linked package.<br><br>For example:<br><br><pre><code>exclude_patterns = ["**/tests/**"]</code></pre>   |  `[]` |
